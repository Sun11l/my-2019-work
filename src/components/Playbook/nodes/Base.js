...
export const Model = joint.shapes.basic.Generic.extend(_.extend(
  ...
  joint.shapes.basic.PortsModelInterface,
  {
    ...
    initEvent: function () {
      ...
      this.listenTo(this, 'change:custom_code', this.hasCustomBlockCode)
      this.listenTo(this, 'change:has_custom_block', this.notifyPanel)
    },
    ...
    hasCustomBlockCode: function (block, e) {
      console.log('hasCustomBlockCode')
      if (e === '') {
        this.set('has_custom_block', false)
      } else {
        block.get('type') !== 'coa.FunctionBlock' && this.set('has_custom_block', true)
      }
      this.markCodeChange()
    },
    // 当编辑器代码被更改时(更改或者回滚代码)，更新panel
    notifyPanel: function (t, e) {
