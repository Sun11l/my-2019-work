...
export default Base.extend({
  ...
  render: function () {
    var t = this.list_data.length > 9
    // eslint-disable-next-line no-mixed-operators
    var e = !!(!this.enabled || this.enabled && !this.freeform)
  ...
  showTip: function (t) {
    if (this.enableTooltip && this.value && !this.tooltip) {
      ...
      var i = $('input.widget-value', this.$el).val()
  ...
  updateSize: function () {
    ...
    if (this.$content.height() < this.trackHeight) {
      this.$track.hide()
      this.show = !1
    } else {
      this.$track.show()
  ...
  handleWheel: function (t) {
    if (this.show) {
      ...
      var i = this.$handle.position().top - 32 * e
      i < 0 ? i = 0 : i > this.trackHeight - this.$handle.height() && (i = this.trackHeight - this.$handle.height())
      this.$handle.css({
        top: i,
      })
      this.updateScroll()
