...
<div class="playbook-meta">
  ...
  <div class="inputs name"></div>
  <div class="inputs tags"></div>
  <div class="clear"></div>
...
export default Backbone.View.extend({
  ...
  initialize: function () {
    ...
    this.listenTo(this.dispatcher, 'settings:close', this.collapse)
    this.render()
  },
  render: function () {
    ...
    this.$el.find('.inputs.name').append(this.nameInput.render().el)

    this.tagsInput = new Input({
      model: this.playbook,
      field: 'tags',
      placeholder: 'Playbook tags',
      label: 'tags',
    })
    this.$el.find('.inputs.tags').append(this.tagsInput.render().el)
    this.$el.find('#description').append(this.playbook.get('description'))
    return this
  },
  // 这块是控制面板的显示和隐藏的
