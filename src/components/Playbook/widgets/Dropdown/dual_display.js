...
export default Base.extend({
  ...
  initialize: function (t) {
    ...
    this.display = o !== -1 ? this.list_data[o].display : this.value
    this.search = ''
  ...
  open: function () {
    ...
    this.$el.find('.dropdown-menu').slideDown(0)
    this.enableBodyClick()
  },
  close: function () {
    this.isOpen = !1
    this.$el.removeClass('open').removeClass('error')
    this.$el.find('.dropdown-menu').slideUp(0)
    this.disableBodyClick()
  },
  select: function (t) {
    this.value = $(t.currentTarget).data('value')
    this.display = $(t.currentTarget).data('display')
    this.useDisplay ? $('input.widget-value', this.$el).val($(t.target).text()) : $('input.widget-value', this.$el).val(this.value)
