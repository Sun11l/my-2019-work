...
import CollectionSearchItems from './collections/SearchItems'
...
const dragAbout = {
  ...
  canvasMouseDown: function (t) {
    var e = this
    if (this.playbook.get('vueObj')) {
      var vue = this.playbook.get('vueObj')
      vue.isDisplay = false
    }
  ...
  canvasMouseUp: function (t) {
    ...
    if (t.target.nodeName === 'svg' && !this.cancelClick) {
      ...
      this.dispatcher.trigger('settings:close')
...
export default Backbone.View.extend({
  ...
  render () {
    ...
    var json = null
    // 获取保存的节点数据 json
    this.playbook.get('ispreview') ? (json = this.playbook.get('data')) : (json = this.playbook.get('json'))
      ...
      _.each(_.sortBy(json.joint.cells, 'order'), (cell) => {
        ...
        switch (type) {
          ...
          case 'coa.Filter':
            ...
            nodeFilter.outputs = new CollectionOutputs(cell.outputs)
            nodeFilter.SerchItems = new CollectionSearchItems(cell.SerchItems)
          ...
          case 'coa.Decision':
            ...
            nodeDecision.outputs = new CollectionOutputs(cell.outputs)
  ...
  initJointInstance () {
    ...
    this.paper = new joint.dia.Paper({
      ...
      // drawGrid画布的背景 为true的时候背景为很多小点点
      drawGrid: false,
  ...
  cellMouseClick: function (target, event) {
    ...
    this.playbook.getnodeName(target)
    this.clearSelector()
