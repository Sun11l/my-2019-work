...
const codeAbout = {
  ...
  revertCode: function () {
    ...
    if (codeView === 'full') {
      t = {
        title: '确定恢复代码?',
        message: '更改的代码将被删除，并将恢复节点功能',
        confirm: '确定',
      }
      e = _.bind(this.revertFullCode, this)
    } else {
      if (codeView === 'global') {
        t = {
          title: '确定恢复代码?',
        ...
        e = _.bind(this.revertGlobalCode, this)
      } else {
        t = {
          title: '确定恢复代码?',
          message: '更改的代码将被删除，并将恢复节点功能',
        ...
        e = _.bind(this.revertBlockCode, this)
      }
    }
  ...
  revertBlockCode: function () {
    ...
    if (block) {
      if (codeview === 'block') {
        block.set('custom_code', '')
      } else {
        if (codeview === 'callback') {
          block.set('custom_callback', '')
        } else {
          codeview === 'join' && block.set('custom_join', '')
        }
      }
...
const playbookAbout = {
  ...
  doSave: function (t, n) {
    ...
    this.playbook.save(this.canvas.graph.toJSON(), {
      ...
      success: function () {
        that.COA_ON_CLOSE && that.COA_ON_CLOSE('save')
      },
    })
  },
...
export default Backbone.View.extend({
  ...
  initialize () {
    ...
    console.log(this.playbook.get('id'))
    if (this.coa.get('editMode')) {
  ...
  render () {
    ...
    if (this.firstLoad) {
      ...
      // this.coa.trigger('change:editMode')
    }
