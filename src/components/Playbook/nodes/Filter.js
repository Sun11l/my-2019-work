...
import { Model, View } from './base.js'
...
import CollectionSearch from '../collections/SearchItems'
...
export const FilterModel = Model.extend(
  ...
  <g class="scalable"></g> 
  <polygon class="background"></polygon>
  <text class="number"/>
  ...
  `,
      defaults: _.defaultsDeep(
        {
          ...
          attrs: {
            ...
            '.background': {
              fill: config.colors.background_dark,
              ...
              rx: 5,
              ry: 5,
              refPoints: '10,10 10,0 20,10 10,20',
              filter: {
                name: 'dropShadow',
                args: {
                  dx: 2,
            ...
            'g.icon image': {
              'xlink:href': `${config.staticPrefix}/block_filter_diy.svg`,
              y: 25,
              x: 10,
              width: 31,
              height: 31,
      ...
      initialize: function () {
        ...
        this.SerchItems = new CollectionSearch()
        ...
        this.on('change:status', function (t, e) {
          ...
          if (e === 'active') {
            i = this.coaSettings.strokeColors[e]
          } else {
            if (n) {
          ...
          if (e === 'warn' || e === 'error') {
            ...
            t.attr('g.error/opacity', 0)
          }
          t.attr('.background/stroke', i)
        })
      ...
      addSearch: function () {
        // 暂时只能添加一组search
        if (!this.SerchItems.length) {
          this.SerchItems.add(new Search())
        }
      },
      ...
      removeOutput: function (t, e, i) {
        ...
        _.each(n, function (t) {
          ...
          if (n > e) {
            i.port = 'out-' + (n - 1)
            i.selector =
              'g:nth-child(1) > g:nth-child(2) > g:nth-child(' +
              (e - 1) +
              ') > circle:nth-child(1)'
            t.set('source', i)
          } else if (n === e) {
      ...
      generateMenuData: function () {
        ...
        this.menuData = []
        this.outputs &&
          _.each(this.outputs.models, function (n, s) {
            var o = {
            ...
            _.each(n.conditions.models, function (t) {
              ...
              if (e.indexOf(':action_result.') !== -1) {
                ...
                _.startsWith(s, 'filtered-data:') &&
                  (s = s.substring(s.lastIndexOf(':') + 1))
                a.push(s)
              }
              if (n.indexOf(':action_result.') !== -1) {
                var s1 = n.substring(0, n.indexOf(':action_result.'))
                _.startsWith(s1, 'filtered-data:') &&
                  (s1 = s1.substring(s1.lastIndexOf(':') + 1))
                a.push(s1)
              }
              (l || n.match(i) || e.match(i)) && (l = true)
            })
            ...
            a.length > 0 &&
              _.each(a, function (i) {
                var n = {
                  name:
                    e + ' condition_' + (s + 1) + ' ' + i + ' filtered results',
      ...
      getMenuData: function (t, e) {
        ...
        _.each(o, function (i) {
          ...
          if (n && (n.id === t.id || e.isSuccessor(n, t))) {
            s.push(a - 1)
            s = _.unique(s)
          }
        })
      ...
      renamePatterns: function (t) {
        var e =
          '^(filtered-data:)(' + t + ')(:condition_[0-9]+:[a-zA-Z0-9_]+:)'
        return [RegExp(e)]
      },
      ...
      updateOutputVarNames: function (t, e) {
        _.each(this.outputs.models, function (i, n) {
          _.each(i.conditions.models, function (i, n) {
            ...
            _.each(t, function (t) {
              return s.match(t)
                ? ((s = s.replace(t, '$1$2' + e)), i.set('value', s), false)
                : void 0
            })
            _.each(t, function (t) {
              return o.match(t)
                ? ((o = o.replace(t, '$1$2' + e)), i.set('param', o), false)
                : void 0
            })
          })
        })
      ...
      validateConfig: function (t, e, i, n) {
        ...
        _.each(this.outputs.models, function (t, o) {
          _.each(t.conditions.models, function (t, a) {
            ...
            var param = t.get('param')
            if (value && !s._checkConfigValue(value, e, i, n)) {
              s.warnings.push('Invalid data path on condition ' + (o + 1))
              s.field_warnings.push(value)
            }
            if (param && !s._checkConfigValue(param, e, i, n)) {
              s.warnings.push('Invalid data path on condition ' + (o + 1))
              s.field_warnings.push(param)
            }
            value === '' &&
            param === '' &&
                s.warnings.push(
                  'No parameters configured on condition ' + (o + 1)
                )
          })
        })
      },
      ...
      toJSON: function () {
        ...
        t.outputs = this.outputs.toJSON()
        t.SerchItems = this.SerchItems.toJSON()
        return t
      },
